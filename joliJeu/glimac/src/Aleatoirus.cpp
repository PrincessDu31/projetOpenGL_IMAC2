...
#define WEAPON_UPGRADE 20
#define WEAPON_DOWNGRADE 15
...
void Aleatoirus::doAction (MainCaracter m) {
	switch(type){
		...
		case MALUS:
			...
			break;
		case WEAPON_UPGRADE:
			int attackUpgrade = rand() % WEAPON_UPGRADE;
			int defenseUpgrade = rand() % WEAPON_UPGRADE;
			m.setAttack(attackUpgrade + m.getAttack());
			m.setDefense(defenseUpgrade + m.getDefense());
			break;
		case WEAPON_DOWNGRADE:
			int attackDowngrade = rand() % WEAPON_DOWNGRADE;
			int defenseDowngrade = rand() % WEAPON_DOWNGRADE;
