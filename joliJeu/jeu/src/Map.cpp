...
void Map::loadMapFromPPM (std::string mapFile) {
	...
	if(file == NULL){
		...
		while (!file.eof()) {
			...
			mapElements.addValueTab((Pixel(rUnsigned,gUnsigned,bUnsigned)));
			mapElements.addElementMap(Pixel(rUnsigned,gUnsigned,bUnsigned).moyennePixels());
		}
